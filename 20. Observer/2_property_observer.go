...
func (t *TrafficManagement) Notify(data interface{}) {
	if pc, ok := data.(PropertyChanged); ok {
		if pc.Value.(int) >= 16 {
			...
			// we no longer care
			t.o.Unsubscribe(t) // subs *list.List , observable copy value, but subs still point to the same list
		}
	}
}
...
func main() {
	...
	for i := 16; i <= 20; i++ { // after age is 16, traffic management will unsubscribe itself
