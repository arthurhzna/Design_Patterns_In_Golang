...
func DrawPoints(owner RasterImage) string { //<---- business logic, depends on RasterImage interface
	maxX, maxY := 0, 0
...
func VectorToRaster(vi *VectorImage) RasterImage { //not adapter, but a wrapper that adapts the VectorImage to the RasterImage interface
	adapter := vectorToRasterAdapter{} // adapter
