...
func main() {
	...
	for ok := true; ok; ok = state != exitState {
		...
		for i := 0; i < len(rules[state]); i++ {
			tr := rules[state][i]
			fmt.Println(strconv.Itoa(i), ".", tr.Trigger) //< for loop value of map rules[state]
		}

		input, _, _ := bufio.NewReader(os.Stdin).ReadLine() //get input from user
...
//all copy value, not pointer
//state is a copy of the state machine
//tr is a copy of the trigger
//state = tr.State is a copy of the state machine
// rules is a map of the state machine
