...
func GetSingletonDatabase() Database {
	once.Do(func() { //just run once, when GetSingletonDatabase twice, it will not run again
...
func GetTotalPopulationEx(db Database, cities []string) int { //Database is interface, so we can pass in any implementation of the interface, SOLID principle
	result := 0
	for _, city := range cities {
